...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (isset($fields['image'])) {
            // if (!File::isDirectory(public_path('/uploads/users'))) {
            //     File::makeDirectory(public_path('/uploads/users'), 0777, true, true);
            // }
            // $imageName =  time() . $fields['image']->getClientOriginalName();
            // $path = '/uploads/users/' .  $imageName;
            // $image = Image::make($fields['image'])->save(public_path($path));
    ...
    public function seflUpdate(Request $request, $user_id) // for user update
    {
        if (auth()->user()->id == $user_id) {
            ...
            if (isset($fields['image'])) {
                // if ($user->image !== "/uploads/users/avatar.png") {
    ...
    public function update(Request $request, $user_id)
    {
        ...
        if (isset($fields['image'])) {
            ...
            // $imageName =  time() . $fields['image']->getClientOriginalName();
            // $path = '/uploads/users/' .  $imageName;
            // $image = Image::make($fields['image'])->resize(100, 100)->save(public_path($path));
